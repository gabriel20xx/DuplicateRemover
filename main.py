...
RESOLUTION_FOLDERS = [os.path.join(BASE_DIR, folder) for folder in ["720p", "1080p", "1080p.x265", "2160p"]]
        ...
        if os.path.isdir(dir_path):
            ...
            if resolution_match:
                ...
                if resolution not in RESOLUTION_PRIORITY:
                    print(f"Skipping folder '{dir_name}' as no valid resolution tag or parent folder resolution found.")
                    continue
...
for (title, year), versions in folders_by_title.items():
    ...
    keep = sorted_versions[0]  # Keep the first (highest-resolution, BluRay-preferred) version

    # Mark all other versions for deletion if they have lower resolution or are WEBRip of the same resolution
    for version in sorted_versions[1:]:
        if RESOLUTION_PRIORITY[version[1]] < RESOLUTION_PRIORITY[highest_resolution] or (
            version[1] == highest_resolution and version[2] != "BluRay"
        ):
